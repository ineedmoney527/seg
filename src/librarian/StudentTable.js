...
import {styled, tableCellClasses, TextField, Paper,Stack, Box, TablePagination} from "@mui/material";
...
const columns = [
    { field: 'studentId', headerName: 'ID', flex: 1,headerAlign: "center" },
    { field: 'name', headerName: 'Name', flex: 1.5,headerAlign: "center" },
    { field: 'email', headerName: 'Email', flex: 2,headerAlign: "center" },
    { field: 'limitLeft', headerName: 'Limit Left', type: 'number', flex: 1,headerAlign: "center" },
];
        ...
        <Box sx={{ width: '100%' , height:'400px'}} className={"boxStudent"}>
            <Paper sx={{ width: '100%', mb: 2 }} className={"paperStudent"}>
                <Stack direction="row" spacing={10} className={"studentHeader"}>
                    <h2>Student Table</h2>
                    <TextField label={"Search Student / Lecturer"}
                               variant="outlined"
                               size="small"
                               value={searchTerm}
                ...
                <div style={{ height: 400, width: '750px' }}>
                    <StyledDataGrid
                        ...
                        //     const selectedStudent = studentsData.find((student) => student.id === ids[0]);
                        //     onSelectStudent(selectedStudent?.studentId, selectedStudent?.name, selectedStudent?.limitLeft);
                        // }}
                    />
                </div>
            </Paper>
