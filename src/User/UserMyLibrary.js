            ...
            <Box sx={{ width: '100%',height:'auto', marginLeft: 'auto', marginRight: 'auto', marginTop: '20px',display:'flex',flexDirection:'column',overflow:'auto' }}>
                ...
                <Box role="tabpanel" hidden={value !== 0}>
                    ...
                    {books
                        ...
                        .map((book, index) => (
                            <Box key={index} sx={{ display: 'flex', flexDirection: 'row',marginLeft:'60px' }}>
                                {books.slice(index, index + 5).map((book, idx) => (
                                    <Box key={idx} sx={{ width: '200px', height: '250px', margin: '10px', borderRadius: '1spx', overflow: 'hidden' }}>
                                        <Box sx={{ width: '200px', height: '200px', backgroundColor: '#F4F4F4', display: 'flex', justifyContent: 'center', alignContent: 'center', alignItems: 'center' }}>
                                            <img src={book.imgPath} alt={book.title} style={{ width: '130px', height: '180px' }} />
                                        </Box>
                                        <Box>
                                            <Typography style={{ fontSize: '14px' }}>Due Date: 30/03/24</Typography>
                                            <Typography style={{ fontSize: '14px' }}>Left 3 days</Typography>
                                        </Box>
                                    </Box>
                                ))}
                            </Box>
                        ...
                        .map((book, index) => (
                            <Box key={index} sx={{ display: 'flex', flexDirection: 'column',marginLeft:'60px' }}>
                                {sampleWishList.slice(index, index + 5).map((book, idx) => (
                                    <Box key={idx} sx={{ width: '80%', height: '100px', margin: '10px', borderRadius: '10px', overflow: 'hidden',backgroundColor:'#F4F4F4',  boxShadow: '0 2px 4px rgba(0, 0, 0, 0.5)' }}>
                                        <Box>
                                            ...
                                            <Box sx={{display:'flex',flexDirection:'row'}}>
                                                <Typography style={{ fontSize: '14px',fontWeight:'bold' }}>Author:</Typography>
                                                <Typography style={{ fontSize: '14px' }}>{book.author}</Typography>
                                            </Box>
                                            <Box sx={{display:'flex',flexDirection:'row'}}>
                                                <Typography style={{ fontSize: '14px',fontWeight:'bold' }}>Publication year:</Typography>
                                                <Typography style={{ fontSize: '14px' }}>{book.publicationYear}</Typography>
                                            </Box>
                                            <Box sx={{display:'flex',flexDirection:'row'}}>
                                                <Typography style={{ fontSize: '14px',fontWeight:'bold' }}>Reason:</Typography>
                                                <Typography style={{ fontSize: '14px' }}>{book.reason}</Typography>
                                            </Box>
                            ...
                            .map((book, index) => (
                                <Box key={index} sx={{ display: 'flex', flexDirection: 'column',marginLeft:'60px' }}>
                                    {sampleReview.slice(index, index + 5).map((book, idx) => (
                                        <Box key={idx} sx={{ width: '80%', height: '150px', margin: '10px', borderRadius: '10px', overflow: 'hidden',backgroundColor:'#F4F4F4',  boxShadow: '0 2px 4px rgba(0, 0, 0, 0.5)' }}>
                                            <Box sx={{display:'flex',flexDirection:'row'}}>
                                                <Box sx={{display:'flex',marginRight:'20px'}}>
                                                    <img src={book.imgPath} alt={"review-book-cover"} style={{width:'100px',height:'150px'}}/>
                                                </Box>
                                                <Box sx={{display:'flex',flexDirection:'column'}}>
                                                    ...
                                                    <Box sx={{display:'flex',flexDirection:'row',marginBottom:'10px'}}>
                                                        ...
                                                        <Rating name="read-only" value={parseFloat(book.reviewRating)} readOnly
                                                                precision={0.5}/>
                                                    </Box>
                                                    <Box sx={{display:'flex',flexDirection:'row'}}>
